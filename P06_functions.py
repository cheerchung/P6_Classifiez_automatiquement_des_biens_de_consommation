...
def display_TSNE(bof=list, title=list, cat=list, p=35):
    """This function transform Bag-of-Feature dataframe or ndarray to tsne matrix,
    then display all"""
    tsne = manifold.TSNE(
    ...
    i = 0
    for bof_ in bof:
        tmp = tsne.fit_transform(bof_)
        dict_TSNE[i] = pd.concat([pd.DataFrame(tmp, columns=['d1', 'd2']), cat],
    ...
    for i, title_, ax_ in zip(range(n), title, ax):
        sns.scatterplot(data=dict_TSNE[i], x='d1', y='d2', hue='main_cat', ax=ax_,
                        legend=[True if i==(n-1) else False][0])
        ax_.set_title(title_)
    h, l = ax_.get_legend_handles_labels()
    ax_.legend(h,l, bbox_to_anchor=(1,0), loc='lower left')
    plt.show()
...
def pca_ScreePlot(ratio, i=90):
    '''This function draw pca components ratio line plot.'''
